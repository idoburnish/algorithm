...
### 리뷰
처음에 누적합을 생각 못하고, 그냥 냅다 배열 각각에 두개의 포인터를 둬서 더하고 뺴면서 진행하니깐 틀렸다. 
그리고 문제 꼼꼼하게 읽어야겠다. 내멋대로 자연수 범위라고 생각하고 풀어서 고치는데 더 오래 걸렸다. 
범위 보는 연습도 해야될것같다. 21억을 넘어가면 long을 사용해야하는데, 자꾸 int로 생각해서 푸는 습관이 있는 것 같다.
